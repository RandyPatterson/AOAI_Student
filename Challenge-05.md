...
The Retrieval-Augmented Generation (RAG) pattern enhances large language models by allowing them to query and use internal or private data for generating more accurate responses. It works by first retrieving relevant documents or information based on a query, then using this information to guide the model's response. This approach helps the model access specific knowledge not included in its original training, making it especially useful for tasks like customer support and research where private or specialized data is needed.

#### :exclamation:**IMPORTANT**

Your prompts (inputs) and completions (outputs), your embeddings, and your training data are:
...
Document chunking is a technique used in AI, especially in Retrieval-Augmented Generation (RAG) models, to break down large documents into smaller, manageable pieces or “chunks.” This makes it easier for the AI to process and retrieve relevant information. Imagine you have a long book and you want to find specific information quickly. Instead of reading the entire book, you divide it into chapters or sections. Each chunk can then be indexed and searched individually, making the retrieval process faster and more efficient.

In the context of RAG, these chunks are used to enhance the AI’s ability to generate accurate and contextually relevant responses. When a query is made, the AI searches through these smaller chunks to find the most relevant information, which it then uses to generate a coherent and informative answer. This method improves the performance and accuracy of AI models by ensuring they have access to the most pertinent data without being overwhelmed by the volume of information.
...
In this challenge, you will create a Semantic Search Plugin that utilizes an Azure AI Search Index to retrieve information from the Contoso Handbook PDF. The purpose of the plugin is to enable the AI Model to answer questions about your own documents. It achieves this by converting the user's query into an embedding using a Text Embedding model. The embedding is then used to search the AI Search Index for the most relevant information.

## Description

1. In the [Azure Portal](https://portal.azure.com/) search for ```AI Search``` and create it in the same resource group and location as your AI Models. Change the pricing tier to **Basic**.  leave everything else as default then **Review + create**

    Once the AI Search resource is created, navigate to the resource the **URL** and **Admin Key**.  Add these values to the **appsettings.json** file in the reference application.  You will need these values later when configuring AI Search.
...
1. Import documents
    * In Azure AI Studio click on Chat -> Add Your Data -> Add Data Source

        ![Add Data Source](./Resources/images/ch05i01.png)

    * Select Data Source = ```Upload Files```
    * Choose an Existing or create a new Blob Storage Resource in the same resource group and location as your AI Models
